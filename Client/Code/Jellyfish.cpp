...
CJellyfish::CJellyfish(LPDIRECT3DDEVICE9 pGraphicDev)
	...
	, m_fAttackTimeAcc(0.f)
{
	...
	m_ObjTag = L"Jellyfish";
}

CJellyfish::CJellyfish(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
	: CMonster(pGraphicDev)
	, m_ePreState(MOTION_END)
	, m_eCurState(MOTION_END)
	, m_fHeight(0.f)
...
CJellyfish::CJellyfish(const CMonster& rhs)
	: CMonster(rhs)
{
	m_ObjTag = L"Jellyfish";
}
...
HRESULT CJellyfish::Ready_Object(void)
{
...
_int CJellyfish::Update_Object(const _float & fTimeDelta)
{
...
void CJellyfish::LateUpdate_Object(void)
{
	CMonster::Billboard();
	Engine::CGameObject::LateUpdate_Object();
}

void CJellyfish::Render_Obejct(void)
{
	if (!m_bRenderOFF)
		CMonster::Render_Obejct();
}

HRESULT CJellyfish::Add_Component(void)
{
	...
	m_pAnimtorCom->Add_Component(L"Proto_JellyfishBossIDLE_Texture");
	m_pAnimtorCom->Add_Component(L"Proto_JellyfishBossATTACK_Texture");
	m_pAnimtorCom->Add_Component(L"Proto_JellyfishBossHIT_Texture");
	m_pAnimtorCom->Add_Component(L"Proto_JellyfishBossDIE_Texture");

	return S_OK;
}

void CJellyfish::Target_Follow(const _float & fTimeDelta)
{
	// 플레이어 따라가기
...
void CJellyfish::OnHit(const _float & fTimeDelta)
{
	if (!m_bHit)
		return;
...
void CJellyfish::CollisionEvent(CGameObject* pObj)
{
...
void CJellyfish::Motion_Change()
{
	if (m_ePreState != m_eCurState)
	{
		...
		{
		case IDLE:
			m_pAnimtorCom->Change_Animation(L"Proto_JellyfishBossIDLE_Texture");
			break;

		case ATTACK:
			Engine::StopSound(SOUND_BROWNDBAT);
			Engine::Play_Sound(L"M_BrownBat_Attack.mp3", SOUND_BROWNDBAT, 1.f);
			m_pAnimtorCom->Change_Animation(L"Proto_JellyfishBossATTACK_Texture");
			break;

		case HIT:
			Engine::StopSound(SOUND_BROWNDBAT);
			Engine::Play_Sound(L"M_BrownBat_Hit.mp3", SOUND_BROWNDBAT, 1.f);
			m_pAnimtorCom->Change_Animation(L"Proto_JellyfishBossHIT_Texture");
			break;

		case DIE:
			Engine::StopSound(SOUND_BROWNDBAT);
			Engine::Play_Sound(L"M_BrownBat_Die.mp3", SOUND_BROWNDBAT, 1.f);
			m_pAnimtorCom->Change_Animation(L"Proto_JellyfishBossDIE_Texture");
			break;
		}
...
CJellyfish * CJellyfish::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CJellyfish *	pInstance = new CJellyfish(pGraphicDev, vPos);
...
CJellyfish * CJellyfish::Create(CMonster * pMonster)
{
	CJellyfish *	pInstance = new CJellyfish(*pMonster);
