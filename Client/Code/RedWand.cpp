#include "stdafx.h"
#include "..\Header\RedWand.h"
#include "Export_Function.h"
#include "ParticleMgr.h"
#include "BulletMgr.h"

CRedWand::CRedWand(LPDIRECT3DDEVICE9 pGraphicDev)
	: CWeapon(pGraphicDev)
{
	m_ObjTag = L"RadWand";
}

CRedWand::CRedWand(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
	: CWeapon(pGraphicDev)
{
	m_vPos = vPos;
	m_ObjTag = L"RadWand";
...
HRESULT CRedWand::Ready_Object(void)
{
...
HRESULT CRedWand::Add_Component(void)
{
	...
	pComponent = m_pTextureCom = dynamic_cast<CTexture*>(Clone_Proto(L"Proto_RadWand1Texture"));
	NULL_CHECK_RETURN(m_pTextureCom, E_FAIL);
	m_mapComponent[ID_STATIC].insert({ L"Proto_RadWand1Texture", pComponent });
	m_textureTag = L"Proto_RadWand1Texture";
...
void CRedWand::CollisionEvent(CGameObject * pObj)
{
...
void CRedWand::Charge(const _float & fTimeDelta)
{
...
void CRedWand::Attack(const _float & fTimeDelta)
{
	if (true == m_bAttack)
	{
...
_int CRedWand::Update_Object(const _float & fTimeDelta)
{
...
void CRedWand::LateUpdate_Object(void)
{
...
void CRedWand::Render_Obejct(void)
{
...
CRedWand * CRedWand::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos)
{
	CRedWand*		pInstance = new CRedWand(pGraphicDev, vPos);
...
void CRedWand::Free(void)
{
	CGameObject::Free();
}

void CRedWand::LevelUp()
{
}
