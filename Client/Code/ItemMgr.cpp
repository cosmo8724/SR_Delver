...
HRESULT CItemMgr::Ready_Proto()
{
	...
	FAILED_CHECK_RETURN(Engine::Ready_Proto(L"Proto_Shield_Texture", CTexture::Create(m_pGraphicDev, L"../Bin/Resource/Texture/Item/Equipment/shield0_%d.png", TEX_NORMAL, 2)), E_FAIL);
...
HRESULT CItemMgr::Add_GameObject(CLayer * pLayer)
{
	...
	//Dagger
	pGameObject = CDagger::Create(m_pGraphicDev, _vec3({ 40.f, 1.f, 40.f }));
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
	...
	pGameObject = CArmor::Create(m_pGraphicDev, _vec3({ 7.f, 1.f, 4.f }), 1);
	NULL_CHECK_RETURN(pGameObject, E_FAIL);
