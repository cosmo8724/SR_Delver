...
CLeftHand::CLeftHand(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos, _int texId)
...
HRESULT CLeftHand::Ready_Object(void)
{
...
void CLeftHand::LateUpdate_Object(void)
{
...
void CLeftHand::Render_Obejct(void)
{
	if (m_eState != STATE_GROUND)
		return;
...
HRESULT CLeftHand::Add_Component(void)
{
...
CLeftHand * CLeftHand::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 vPos, _int texId)
{
	CLeftHand*	pInstance = new CLeftHand(pGraphicDev, vPos, texId);
...
void CLeftHand::Free(void)
{
	CGameObject::Free();
}

void CLeftHand::CollisionEvent(CGameObject * pObj)
{
	if (STATE_GROUND == m_eState)
	{
